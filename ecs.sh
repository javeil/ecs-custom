...
while [ "$#" -gt 0 ]; do
    ...
    -h)
        if [ "$en_status" = true ]; then
            ...
            echo "-r     可选项，可指定回程路由测试中的三网IPV4地址，可选 b g s c t 分别对应 北京、广州、上海、成都、天津 的三网地址，如 -r t 指定测试天津地址"
...
test_area_t=("天津电信" "天津联通" "天津移动")
test_ip_t=("219.150.32.132" "202.99.96.68" "211.137.160.5")
...
nexttrace_route_script() {
    ...
    elif [ -n "$route_location" ] && [[ "$route_location" =~ ^(b|g|s|c|t)$ ]]; then
    ...
    if [[ -z "${ip4}" ]] && [[ -n "$ip6" ]] && [ -z "$route_location" ]; then
        ...
        # 注: 天津暂无IPv6三网节点，IPv6-only环境仍使用广州IPv6地址
    fi
...
all_script() {
    ...
    if [ "$1" = "B" ]; then
        if [[ -z "${CN}" || "${CN}" != true ]]; then
            ...
            nexttrace_route_script test_area_t[@] test_ip_t[@] >${TEMP_DIR}/fscarmen_route_output.txt &
        ...
        if [[ -z "${CN}" || "${CN}" != true ]]; then
            ...
            spiritlhl_script
            backtrace_script
            nexttrace_route_script test_area_t[@] test_ip_t[@]
            wait
            ecs_net_all_script
        else
            _yellow "Concurrently downloading files..."
